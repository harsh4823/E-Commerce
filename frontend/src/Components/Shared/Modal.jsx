      ...
      <Dialog open={open} onClose={() => setOpen(false)} className="relative z-10">
        ...
        <div className="fixed inset-0 overflow-hidden">
            <div className='absolute inset-0 overflow-hidden'>
                <div className='pointer-events-none fixed right-0 flex max-w-full pl-10 inset-y-0'>
                <DialogPanel transition className='pointer-events-auto relative w-screen max-w-[600px] transform transition duration-500 ease-in-out data-closed:translate-x-full sm:duration-700'>
                ...
                <button onClick={()=>setOpen(false)}><RxCross1 className='text-slate-800 text-2xl'/></button>
                </div>
                {children}
                </div>                  
                </div>
